...
def generateIRX(scanName, configFile=None):
    ...
    with io.open(stdoutFile, 'wb') as writer, io.open(stdoutFile, 'rb', 1) as reader:
        if(configFile):
            ...
            process = subprocess.Popen(["appscan", "prepare", "-n", scanName], stdout=writer, shell=True)
...
def waitForScan(scanId, timeoutMins=360, targetStatus="Ready", printStatusEveryMins=5):
    try:
        curStatus = getScanStatus(scanId)
    except UnauthenticatedException:
        if(loginASoC(key_id, key_sec)==False):
            raise UnauthenticatedException
        curStatus = getScanStatus(scanId)
...
class UnauthenticatedException(Exception):
    def __init__(self):
        Exception.__init__(self,"You are not logged into AppScan on Cloud") 
